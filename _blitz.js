...
// const baseDirectory = dirname(require.main?.filename || "");
var commandsAsync = [
  "delay",
  "input",
  "waitkey",
  ...
  "loadanimimage",
  "writebytes",
].map((res) => `_${res}`); //'getkey', 'getmouse', 'keyhit', 'keydown',
...
function traverseDir(dir, files = []) {
  files = [];
  readdirSync(folder.base + "/" + dir).forEach((file) => {
    let fullPath = join(dir, file);
    if (lstatSync(folder.base + "/" + fullPath).isDirectory()) {
...
function readAsyncCommands() {
  ...
  commandsAsync.push(...files.map((/** @type {string} */ filename) => filename.replace(/^.*[\\/]([!a-zA-Z0-9$#%]*?)\.js$/g, "_$1").toLowerCase()));
}
readAsyncCommands();

function readCommands() {
  commandsFiles = traverseDir("commands/");
  return commandsFiles.map((/** @type {string} */ filename) => filename.replace(/^.*[\\/]([!a-zA-Z0-9$#%]*?)\.js$/g, "$1").toLowerCase());
}
...
function printCommands() {
  ...
  return commandsFiles
    .map((filename) => {
      return readFileSync(folder.base + "/" + filename);
    })
    .join("\n");
}

function printTools() {
  const tools = readdirSync(folder.base + "/static/js/");
  return tools.map((filename) => readFileSync(folder.base + "/static/js/" + filename)).join("\n");
}
 ...
 * @param {number} num
 * @param {RegExp} rx
 * @returns {[string, RegExpMatchArray | null]}
 */
 ...
 * @param {any} body
 * @param {number} num
 * @param {RegExpMatchArray | null} arr
 */
...
function parseCommands(result) {
  result = result.replace(
    /(\b[a-zA-Z_][a-zA-Z0-9_$]*?\b)[$#%]?/gim,
    (/** @type {any} */ res, /** @type {any} */ a1, /** @type {any} */ a2, /** @type {any} */ a3, /** @type {any} */ a4) => {
  ...
  while (comWhile < 10) {
    result = result.replace(
      comRx,
      (/** @type {any} */ res, /** @type {string} */ a1, /** @type {string} */ a2, /** @type {any} */ a3, /** @type {any} */ a4) => {
        a1 = a1;
        a2 = a2 || "";
  ...
  result = result.replace(
    /\(\b0([a-zA-Z][a-zA-Z0-9_$]*?)\b( *)\b([_a-zA-Z][a-zA-Z0-9_$#%]*?)\b/gim,
    (/** @type {any} */ res, /** @type {any} */ a1, /** @type {any} */ a2, /** @type {string} */ a3) => {
  ...
  result = result.replace(
    /\b_([a-z0-9_$]*?\b) *\( *(\b[^,\r\n]*\b) *(,? *?)(.*) *\);/gim,
    (/** @type {any} */ res, /** @type {string} */ a1, /** @type {any} */ a2, /** @type {any} */ a3, /** @type {any} */ a4) => {
...
function parsePart(part, directory) {
  let result = part;
  let /** @type {RegExpMatchArray | null} */ quotes = null;
  let /** @type {RegExpMatchArray | null} */ comments = null;
  let /** @type {RegExpMatchArray | null} */ js = null;

  result = result.replace(/include *"(.*?)"/gim, (/** @type {any} */ res, /** @type {any} */ a1) => {
    try {
      return readFileSync(`${process.cwd()}/public${directory}/${a1}`);
    } catch (err) {
      return "";
    }
  });
  ...
  result = result.replace(
    /\bif *(.+?) *$/gim,
    (/** @type {any} */ res, /** @type {{ toLowerCase: () => string | string[]; match: (arg0: RegExp) => never[]; }} */ a1) => {
  ...
  result = result.replace(/(\b[a-z][a-zA-Z0-9_$#%]*?\b)/gim, (/** @type {any} */ res, /** @type {string} */ a1) => {
  ...
  result = result.replace(
    /\bfunction *(.*?) *\((.*?)\);([\w\W]*?)\bend function/gim,
    (/** @type {any} */ res, /** @type {any} */ a1, /** @type {string} */ a2, /** @type {string} */ a3) => {
      const varListEx = a2
        .replace(/[ $#%]*/gim, "")
        .replace(/(\b[a-z][a-zA-Z0-9_]*?) *=.*?(,|$)/gim, "$1$2")
        .split(",");
      let varList = [];
      a3 = a3.replace(/^.*?(\b[a-z][a-zA-Z0-9_$#%]*?\b)(?! *\()/gm, (/** @type {any} */ res, /** @type {string} */ a4) => {
        const variable = `${a4}`;
        // if (a1 === 'filegetindex') {
        ...
        if (
          varList.indexOf(variable) === -1 &&
          variableReserved.indexOf(a4) === -1 &&
          variableDouble.indexOf(a4) === -1 &&
          !a4.startsWith("xx") &&
          !a4.startsWith("_") &&
          result.search(typeRx) === -1 &&
          result.search(functionRx) === -1 &&
          result.search(constRx) === -1 &&
          result.search(globalRx) === -1 &&
          varListEx.indexOf(a4) === -1
        ) {
  ...
  const dimGetRx = [];
  const dimSetRx = [];
  result = result.replace(/^dim ([a-zA-Z0-9_]+?)[$#%]? *\((.+?)\);/gim, (/** @type {any} */ state, /** @type {any} */ m1, /** @type {any} */ m2) => {
    dimGetRx.push(new RegExp(`(\\b${m1}\\b) *\\(`, "gm"));
    dimSetRx.push(new RegExp(`^(\\b${m1}\\b) *\\((.*)\\)(.*?) *(?==)`, "gm"));
    return `var ${m1} = new _Dim(${m2});`;
  });
  for (const dimRx of dimSetRx) {
    result = result.replace(dimRx, (/** @type {any} */ res, /** @type {any} */ m1, /** @type {string} */ m2, /** @type {any} */ m3) => {
  ...
  // });
  result = result.replace(/\b(field)\b *(.*);/gim, (/** @type {any} */ res, /** @type {string} */ a1, /** @type {string} */ a2) => {
    return `${a1} ${a2.replace(/(.*?), */gi, "$1;\n" + a1 + " ")}; `;
  });
  result = result.replace(/\b(field|global|local|const)\b *([a-z][a-z0-9_]*?)(?: *= *(.+?));/gim, "$1 $2 = $3;");
  result = result.replace(
    /\b(field|global|local|const)\b *([^=\n\[\]]+?)\[(.*)\];/gim,
    (/** @type {any} */ res, /** @type {any} */ a1, /** @type {any} */ a2, /** @type {string} */ a3) => {
  ...
  result = result.replace(
    /(\bif\b|\bwhile\b|\breturn\b|^[a-z][a-z0-9_]*? *=) *(?:\((.*?)\)|(.*?))( *[{;])/gim,
    (/** @type {any} */ state, /** @type {any} */ m1, m2 = "", m3 = "", /** @type {any} */ m4) => {
      if (m2 + m3 === "") {
        return m1 + m4;
      }
  ...
  result = result.replace(/\bcase\b *(.+)[:\n]/gim, (/** @type {any} */ res, /** @type {string} */ a1) => {
  ...
  result = result.replace(/(\b[a-z][a-z0-9_]*\b) *(?!\()(.+?);/gm, (/** @type {any} */ com, /** @type {string} */ a1, /** @type {any} */ a2) => {
  ...
  result = result.replace(/(\b_?[a-z][a-z0-9_]*\b) *\(/gm, (/** @type {any} */ com, /** @type {string} */ a1) => {
    const resultRx = new RegExp(`async +function +${a1}\\b *\\(`, "i");
    if (result.search(resultRx) > -1 || commandsAsync.indexOf(a1) > -1) {
      return `await ${a1} (`;
    }
  ...
  let varList = [];
  result = result.replace(/(\b[a-z][a-zA-Z0-9_$#%]*?\b)(?! *\()/gm, (/** @type {any} */ res, /** @type {string} */ a1) => {
    const varRx = new RegExp(`var +${a1}\\b`);
    if (
      result.search(varRx) === -1 &&
      varList.indexOf(`var ${a1};`) === -1 &&
      variableReserved.indexOf(a1) === -1 &&
      variableDouble.indexOf(a1) === -1 &&
      !a1.startsWith("xx") &&
      !a1.startsWith("_") &&
      result.indexOf(`class ${a1} {`) === -1
    ) {
      //
      varList.push(`var ${a1};`);
      //return `var ${a1};\n${res}`;
  ...
  result = result.replace(
    /\bfunction await *([a-z][a-z0-9_]*) *\((.*)\)/gm,
    (/** @type {any} */ res, /** @type {any} */ a1, /** @type {any} */ a2) => {
  ...
  for (let d = dataListSearch.length - 1; d >= 0; d--) {
    ...
    result = result.replace(
      ...
      (
        ...
        /** @type {any} */ a1,
        /** @type {string} */ a2,
        /** @type {{ match: (arg0: RegExp) => never[]; replace: (arg0: RegExp, arg1: string) => any; }} */ a3
      ) => {
        a2 = a2.substring(1);
        const ret = `_data("${a2}"`;
        const m2 = (a3.match(/(\bdata *(.*?);)/gim) || [])
          .map((/** @type {string} */ res2) => res2.replace(/^data *|;$/g, "").split(/ *, */))
 ...
 */
export function parseBB(bb, directory) {
