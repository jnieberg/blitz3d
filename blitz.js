...
var fs = require('fs');
var commandsAsync = ['waitkey', 'keyhit', 'keydown', 'delay', 'input'].map((res) => `_${res}`);
var commandsStatic = ['class', 'in', 'of', 'var', 'case', 'select']
var commands = [];
var commandsFiles = [];

function readCommands() {
	commandsFiles = fs.readdirSync('commands/');
	return commandsFiles.map((filename) => filename.replace(/\.js$/g, '').toLowerCase());
}

function printCommands() {
	commands = readCommands();
	return commandsFiles.map((filename) => fs.readFileSync('commands/' + filename)).join('\n');
}
...
function endProgram() {
	return '_debug(\'Program has ended\'); window.stop();';
}
...
function parsePart(part) {
	...
	result = result.replace(/^\.(.+?);$/gm, '$1:');
	result = result.replace(/\bmod\b/gm, '%');
	result = result.replace(/^dim\(([a-zA-Z0-9_$]+?)\((\d+?)\)\);$/gim, 'var $1 = new Array($2);');
	result = result.replace(/^([a-zA-Z0-9_$]+?)\(([a-zA-Z0-9_$]+?)\) *= */gim, '$1[$2] = ');

	result = result.replace(/(\b[a-zA-Z0-9_$]+?)\.([a-zA-Z0-9_$]+?\b)/gim, '$1');
	result = result.replace(/(\b[a-zA-Z0-9_$]+?\b)\\(\b[a-zA-Z0-9_$]+?\b)/gim, '$1.$2');
	...
	result = result.replace(/\$([a-zA-Z0-9]+)/gim, '0x$1');
	...
	result = result.replace(/(\b[a-z_][a-zA-Z0-9_$]*\b) *(\(?)/gm, (com, a1, a2) => {
		if (result.indexOf(`async function ${a1}(`) > -1 || commandsAsync.indexOf(a1) > -1) {
			if (a2) {
				return `await ${a1}${a2}`;
			} else {
				return `await ${a1}()`;
