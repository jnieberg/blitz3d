...
function parsePart(part, directory) {
	...
	[result, js] = backupText(result, 3, /\[\[([\w\W]*?)\]\]/gm);
	...
	result = result.replace(/\bnot *(\b[a-zA-Z0-9_$#%\(\)]+\b)/gim, '_not($1)');
	...
	result = result.replace(/\bfunction *(.*?) *\((.*?)\);([\w\W]*?)\bend function;/gim, (res, a1, a2, a3) => {
		...
		a3 = a3.replace(/^.*?(\b[a-z][a-zA-Z0-9_$#%]*?\b)(?! *\()/gm, (res, a4) => {
			...
			// }
			if (varList.indexOf(variable) === -1 && variableReserved.indexOf(a4) === -1 && variableDouble.indexOf(a4) === -1 && !a4.startsWith('xx') && !a4.startsWith('_') && result.indexOf(`type ${a4};`) === -1 && result.indexOf(`function ${a4} `) === -1 && result.indexOf(`function ${a4}(`) === -1 && varListEx.indexOf(a4) === -1) {
				varList.push(variable);
			}
			return res;
		});
		const varDecl = varList.map(res => `let ${res} = 0;`).join('\n');
	...
	result = result.replace(/^for *(.*?) *= *(.*?) *to *(.*?) step *\-(.*?);/gim, 'for($1=$2; $1>=$3; $1=$1-$4) {');
	result = result.replace(/^for *(.*?) *= *(.*?) *to *(.*?) step *(.*?);/gim, 'for($1=$2; $1<=$3; $1=$1+$4) {');
	result = result.replace(/^for *(.*?) *= *(.*?) *to *(.*?);/gim, 'for($1=$2; $1<=$3; $1+=1) {');
	...
	result = result.replace(/\b(field|global|local)\b *(.+?)(?: *= *(.+?))?;/gim, '$1 $2 = new _Obj($3);');
	const commandsMap = {
		select: 'switch($1) {',
		case: 'break;\ncase $1:',
		repeat: 'do {',
	...
	result = result.replace(/(\bif\b|\bwhile\b|=) *(?:\((.*?)\)|(.*?))( *[{;])/gim, (state, m1, m2 = '', m3 = '', m4) => {
	...
	result = result.replace(/^function *(\b[a-z][a-z0-9]*\b\(.*?\));/gim, 'async function $1 {');
	result = result.replace(/(\b[a-z][a-z0-9]*\b) *(?!\()(.+?);/gm, (com, a1, a2) => {
	...
	result = result.replace(/(\b_?[a-z][a-z0-9]*\b) *\(/gm, (com, a1) => {
		const resultRx = new RegExp(`async * function* ${a1} *\\(`, 'i')
		if (result.search(resultRx) > -1 || commandsAsync.indexOf(a1) > -1) {
			return `await ${a1} (`;
		}
	...
	result = result.replace(/^.*?(\b[a-z][a-zA-Z0-9_$#%]*?\b)(?! *\()/gm, (res, a1) => {
		const push = `${a1}`;
		if (result.indexOf(`var ${push};`) === -1 && result.indexOf(`var ${push} `) === -1 && varList.indexOf(`var ${push};`) === -1 && variableReserved.indexOf(a1) === -1 && variableDouble.indexOf(a1) === -1 && !a1.startsWith('xx') && !a1.startsWith('_') && result.indexOf(`class ${a1} {`) === -1) { //
			varList.push(`var ${push};`);
			//return `var ${push};\n${res}`;
		}
		return res;
	});
	...
	result = result.replace(/\bfunction await *([a-z][a-z0-9]*) *\((.*)\)/gm, (res, a1, a2) => {
		//a2 = a2.replace(/(, *\b[a-z][a-zA-Z0-9_$#%]*?\b)(?! *=)/gim, '$1 = new _Obj()');
		return `function ${a1} (${a2})`;
	});
	result = result.replace(/\bresolve\(await +/gim, 'resolve(');

	result = result.replace(/\b_read *\(([a-z][a-z0-9]*)\);/gim, '_read();');
	result = result.replace(/\b_restore *\(([a-z][a-z0-9]*)\);/gim, '_restore("$1");');
...
while(await _async()) {
	...
	const dataListSearch = (result.match(/^\.\b[a-z][a-z0-9]*\b/gim) || []);//.filter(res => res !== 'xx1xx' && res !== 'xx3xx' && res !== 'default');
	for (let d = dataListSearch.length - 1; d >= 0; d--) {
		const dataResRx = new RegExp(`([\\w\\W]*) *(${dataListSearch[d]}) *([\\w\\W]*)`, 'gim');
...
${m3}`
	...
	result = result.replace(/^\.(\b[a-z][a-z0-9]*\b);/gim, '$1:');
	result = result.replace(/goto *(\b[a-z][a-z0-9]*\b);/gim, '_goto = "$1"; break;')
